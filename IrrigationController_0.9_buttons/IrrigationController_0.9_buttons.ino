...
//Or CLK -> 7, DAT -> 8, Reset -> 9
 // command below is (CLK pin, Data Pin, Reset pin
virtuabotixRTC myRTC(PB12, PB13, PB14); //If you change the wiring change the pins here also
...
int editMode;
int runs = 0;
 const int PROGMEM  selBtnPin = PB5;
 const int PROGMEM upBtnPin = PB8;
 const int PROGMEM dnBtnPin = PB4;
const int PROGMEM checkBoxW = 9;
const int PROGMEM checkBoxH = 9;
int selBtnState = 0;

const int PROGMEM RegData = PB11;
const int PROGMEM RegClk = PB10;
const int PROGMEM RegLatch = PB1;
 ...
 * 0 {{ Datatype (0 = blank, 1 = enable at time, 2 = toggle every time)
 * 1   Enable hour
 * 2   Enable Minute
 * 3   Disable hour
 * 4   Disable minute
 * 5   enable wait hour
 * 6   enable wait minute
 * 7   disable wait hour
 * 8   disable wait minute
 * 9   enable day mon
 * 10   enableday tue
 * 11   enableday wed
 * 12   enableday thurs
...
int EPROMAdder = setupMatrixLocation;
int port = 0;
int setting = 0;
for(port = 0; port<8; port++){
for(setting = 0; setting<16; setting++){
 int stateMatrixTMP = setupMatrix[port][setting];
 setupMatrix[port][setting]= EEPROM.read(EPROMAdder);
  EPROMAdder++;
}
...
pinMode(selBtnPin, INPUT);
pinMode(upBtnPin, INPUT);
pinMode(dnBtnPin, INPUT);
...
if(digitalRead(upBtnPin) == HIGH){
pos--;
delay(200);
  
}
if(digitalRead(dnBtnPin) == HIGH){
pos++;
delay(200);
...
for(setting = 0; setting<16; setting++){
 int stateMatrixTMP = setupMatrix[port][setting];
  EEPROM.update(EPROMAdder, stateMatrixTMP);
  EPROMAdder++;
}
...
void checkBtn(){
//Serial.println(selBtnState);
selBtnState = digitalRead( selBtnPin);
if(selBtnState == HIGH){
...
if(pos>4 and pos<12){
 ...
 if (setupMatrix[portToEdit-1][pos+4]==1){
  setupMatrix[portToEdit-1][pos+4]=0;
 }else{
  setupMatrix[portToEdit-1][pos+4]=1;
 }
...
pos = 0;
 ...
 if(digitalRead(selBtnPin) == HIGH){
 if(editing == 1 or editing ==2 or editing ==5 or editing == 6) {
  editing = 0;
  pos = 0;
 }
...
display.print(setupMatrix[portToEdit-1][1]);
display.print(F(":"));
display.print(setupMatrix[portToEdit-1][2]);
if(pos == 3){
...
display.print(setupMatrix[portToEdit-1][3]);
display.print(F(":"));
display.print(setupMatrix[portToEdit-1][4]);
if(pos == 4){
...
int tmp = setupMatrix[portToEdit-1] [loops+9];
if(tmp == 1){
...
if(pos>4 and pos<12){
  int reletivePos=((pos-5)*15)+5;
display.drawRect(reletivePos, 44,9, 20, WHITE);
 // display.drawPixel(((pos-4)*15)+7,44, SSD1306_WHITE);
...
setupMatrix[portToEdit-1][0] = editMode;
}

if(editing == 3){
setupMatrix[portToEdit-1][1] = pos;
...
display.print(F(":"));
display.print(setupMatrix[portToEdit-1][2]);
setupMatrix[portToEdit-1][2] = pos;
...
setupMatrix[portToEdit-1][3] = pos;
display.print(setupMatrix[portToEdit-1][3]);
display.print(F(":"));
display.print(setupMatrix[portToEdit-1][4]);
if(pos == -1){
...
setupMatrix[portToEdit-1][4] = pos;
...
setupMatrix[port][0] = 0;

setupMatrix[port][1] = 0;

setupMatrix[port][2] = 0;

setupMatrix[port][3] = 0;

setupMatrix[port][4] = 0;

setupMatrix[port][5] = 0;

setupMatrix[port][6] = 0;

setupMatrix[port][7] = 0;

setupMatrix[port][8] = 0;

setupMatrix[port][9] = 1;

setupMatrix[port][10] = 1;

setupMatrix[port][11] = 1;

setupMatrix[port][12] = 1;

setupMatrix[port][13] = 1;

setupMatrix[port][14] = 1;

setupMatrix[port][15] = 1;
